...
class STM:
    ...
    def init_beta(self):
        ...
        if self.init == "spectral":
            self.beta = spectral_init(
                self.documents, self.K, self.V, maxV=5000, verbose=True
            )
    ...
    def E_step(self):
        ...
        print(f"Completed E-Step in {elapsed_time} seconds. \n")
    ...
    def M_step(self, beta_ss, sigma_ss):
        ...
        elapsed_time = np.round((time.time() - start_time), 3)
        print(f"Completed M-Step in {elapsed_time} seconds. \n")
    ...
    def expectation_maximization(self, saving, output_dir=None):
        ...
        print(f"Fit STM for {self.K} topics")
        print("––––––––––––––––––––––––––––––––––––––––––––––––––––––––")
        for _iteration in range(100):
            print(f"E-Step iteration {_iteration}")
            beta_ss, sigma_ss = self.E_step()
            print(f"M-Step iteration {_iteration}")
    ...
    def label_topics(self, topics, n, frexweight=0.5, print_labels=False):
        ...
        for k in K:
            if k > len(problabels): 
                print(f"Skipping topic {k}: out of bounds")
                continue
